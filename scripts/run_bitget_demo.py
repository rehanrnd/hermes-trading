...
from hermes_trading_agent.bitget import build_bitget_setup_report, format_bitget_setup_report, load_bitget_config
...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Run a Hermes Trading Bitget paper/demo session")
    parser.add_argument(
        "--bitget-config",
        type=Path,
        default=Path("state/bitget.json"),
        help="Bitget config JSON file",
    )
    parser.add_argument(
        "--bitget-env-file",
        type=Path,
        default=Path("state/bitget.env"),
        help="Local Bitget env file with API credentials",
    )
    parser.add_argument(
        "--data-csv",
        type=Path,
...
def main() -> None:
    ...
    goal = load_goal()
    config = load_bitget_config(args.bitget_config)
    setup_report = build_bitget_setup_report(config, env_file=args.bitget_env_file)

    print(format_bitget_setup_report(setup_report))
